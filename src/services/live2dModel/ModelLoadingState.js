...
Live2DFactory.live2DModelMiddlewares.unshift((context, next) => {
  ...
  context.live2dModel.factoryContext = context;
...
export class ModelLoadingState {
  ...
  complete(stage, model) {
    if (
      stage !== "settingsJSON" &&
      this.stages.settingsJSON !== StageState.COMPLETED
    ) {
    ...
    if (stage === "settingsJSON") {
      ...
      model.factoryContext.settings.pose &&
        (this.stages.pose = StageState.PENDING);
      model.factoryContext.settings.physics &&
        (this.stages.physics = StageState.PENDING);
    }
  ...
  updateText() {
    this.text = Object.entries(this.stages)
      .map(([stage, state]) => {
