...
import { downloadPCM, downloadWAV, downloadTempFile } from "./js/download.js";
import CryptoJS from "crypto-js";
// import TransWorker from "./js/transcode.worker.js";
import { Base64 } from "js-base64";

const transWorker = new Worker(
  new URL("./js/transcode.worker.js", import.meta.url),
  {
    name: "wastedTime",
...
const APPID = "071af495";
const API_SECRET = "MTRkODIzZDhkODBmZmUwYzJlZGJhYTQ1";
const API_KEY = "2f2456a22ab223212b162534bee8133b";
...
class TTSRecorder {
  ...
  webSocketSend() {
    ...
    console.log((timer = new Date().getTime()), "wsstart");
  ...
  result(resultData) {
    ...
    if (jsonData.code === 0 && jsonData.data.status === 2) {
      console.log((timer = new Date().getTime() - timer), "wsend");
      this.ttsWS.close();
    }
  }
  // 重置音频数据
  ...
  // 音频播放
  audioPlay() {
    console.log((articleTimer = new Date().getTime()), "playstart");
    this.setStatus("play");
    ...
    bufferSource.onended = (event) => {
      ...
      if (this.audioDataOffset < this.audioData.length) {
        ...
        console.log(
          (articleTimer = new Date().getTime() - articleTimer),
          "playend"
        );
        this.audioStop();
      }
  ...
  async start() {
    if (this.audioData.length) {
      ...
      const { wavBuffer, url, voiceBlob, voiceBuffer } = downloadTempFile(
        new DataView(new Int16Array(ttsRecorder.rawAudioData).buffer),
        16000,
        16
      );
      return { wavBuffer, url, voiceBlob, voiceBuffer };
    }
  }
  stop() {
    this.audioStop();
...
export { ttsRecorder, downloadWAV };
...
//   if (checked) {
//     text =
//       'ياخشىمۇسىز، شىنجاڭ ئۇنىۋېرسىتېتى شۈنفېي ئاۋاز كىرىشتۈرۈش سېستىمىسىنى ئىشلىتىشىڭىزنى قىزغىن قارشى ئالىمىز،  مەن گۈلنۇر بولىمەن.';
...
// const replaceStr = (v) => {
//   const replaceReg = /[,，。、……]/g;
//   return v.replace(replaceReg, '<break time="1000ms"/>');
// };

// $('#input_text').change(function () {
//   // console.log(replaceStr(this.value))
//   ttsRecorder.setParams({
//     text: this.value
//   });
// });
